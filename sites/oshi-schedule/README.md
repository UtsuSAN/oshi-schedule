...
- 発売予定表示を確認する場合は、リポジトリルートから架空データをコピーできます: `Copy-Item .\sites\oshi-schedule\examples\ticket-release-preview.json .\sites\oshi-schedule\dist\data\public_snapshot.json -Force`。2026年9月26日の「今日」と9月27日の「今週」に発売予定が表示されます。元に戻すには `git restore .\sites\oshi-schedule\dist\data\public_snapshot.json` を実行します。
